...
describe('FoodController (with Faker)', () => {
    ...
    it('should create food with faker data', async () => {
        ...
        (commandBus.execute as jest.Mock).mockResolvedValue({ id: faker.string.uuid(), ...dto });
    ...
    it('should return all foods', async () => {
        const fakeFoods = Array.from({ length: 3 }, () => ({ id: faker.string.uuid(), name: faker.commerce.productName() }));
        (queryBus.execute as jest.Mock).mockResolvedValue(fakeFoods);
    ...
    it('should return food by id', async () => {
        ...
        (queryBus.execute as jest.Mock).mockResolvedValue(fakeFood);
